...
test('can test creating from key bundle with signer', async () => {
  const keyBytes = new Uint8Array([
  ...
  await alix.conversations.newConversation(bob.address)
  ...
  const convos1 = await alixKeyBundle.conversations.list()
  const convos2 = await alix.conversations.list()
  assert(convos1.length === convos2.length, `The convos length should match`)
