...
test('can stream conversations and messages including reactions', async () => {
  const [alix, bo] = await createClients(2)
  ...
  assert(
    (await bo.conversations.list()).length === 2,
    `convrtdsyion length should be 2`
  )
  // Start message stream
  await bo.conversations.streamAllMessages(async (msg) => {
    messageCallbacks++
  })


  // Send regular messages
  const messageId1 = await group.send('hello group')
  const messageId2 = await dm.send('hello dm')
  await delayToPropogate()
  await delayToPropogate()


  // Send reactions to those messages
  await group.send({
    reactionV2: {
      action: 'added',
      content: '😃', // Reaction V2
  ...
  // Validate message stream callbacks (4: 2 messages + 2 reactions)
  assert(
    messageCallbacks === 4,
    `message stream should have received 4 messages (including reactions), got ${messageCallbacks}`
  )
