...
test('can stream conversations and messages including reactions', async () => {
  ...
  assert(
    ...
    `conversation length should be 2`
  )
  ...
  const messageId1 = await group.send({ text: `hello group` })
  const messageId2 = await dm.send({ text: `hello dm` })
  ...
  assert(
    allBoMessages.length === 4,
    `message stream should have received 4 messages (including reactions), got ${allBoMessages.length}`
  )
