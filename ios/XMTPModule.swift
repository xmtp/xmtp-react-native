...
public class XMTPModule: Module {
	...
	public func definition() -> ModuleDefinition {
		...
		AsyncFunction("sendEncodedContentData") { (clientAddress: String, conversationTopic: String, _: String?, content: [UInt8]) -> String in
		...
		AsyncFunction("decodeMessage") { (clientAddress: String, topic: String, encryptedMessage: String, _: String?) -> String in
	...
	func subscribeToMessages(clientAddress: String, topic: String, conversationID _: String?) async throws {
		guard let conversation = try await findConversation(clientAddress: clientAddress, topic: topic) else {
			return
		}
			...
			do {
				for try await message in conversation.streamMessages() {
					try sendEvent("message", [
						"topic": conversation.topic,
						"conversationID": conversation.conversationID,
						"messageJSON": DecodedMessageWrapper.encode(message),
					])
				}
			} catch {
	...
	func unsubscribeFromMessages(clientAddress: String, topic: String, conversationID _: String?) async throws {
		guard let conversation = try await findConversation(clientAddress: clientAddress, topic: topic) else {
			return
		}
